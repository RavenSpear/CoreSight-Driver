...
static void tpiu_enable_hw(struct csdev_access *csa)
{
	...
	suppportsz = csdev_access_relaxed_read32(csa,TPIU_SUPP_PORTSZ);
	//pr_info("TPIU support port size: %x\n", suppportsz);

	currportsz = csdev_access_relaxed_read32(csa,TPIU_CURR_PORTSZ);
	//pr_info("TPIU current port size: %x\n", currportsz);

	ffsr = csdev_access_relaxed_read32(csa,TPIU_FFSR);
	//pr_info("Formatter and Flush Status Register: %x\n", ffsr);

	ffcr = csdev_access_relaxed_read32(csa,TPIU_FFCR);
	//pr_info("Formatter and Flush Control Register: %x\n", ffcr);

	csdev_access_relaxed_write32(csa, 0x80000000, TPIU_CURR_PORTSZ);
	csdev_access_relaxed_write32(csa, 0x103, TPIU_FFCR);
...
static int tpiu_enable(struct coresight_device *csdev, u32 mode, void *__unused)
{
	...
	dev_info(&csdev->dev, "TPIU enabled\n");
	return 0;
}
...
static int tpiu_disable(struct coresight_device *csdev)
{
	...
	dev_info(&csdev->dev, "TPIU disabled\n");
	return 0;
}
