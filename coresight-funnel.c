...
static int dynamic_funnel_enable_hw(struct funnel_drvdata *drvdata, int port)
{
	...
	funelfunctl = readl_relaxed(drvdata->base + FUNNEL_FUNCTL);
	funnelprictl = readl_relaxed(drvdata->base + FUNNEL_PRICTL);
	pr_info("Funnel Control Register: %x\n", funelfunctl);
	pr_info("Funel Priority Control Register: %x\n", funnelprictl);
done:
			 ...
			 int outport)
{
	...
	if (first_enable)
		dev_info(&csdev->dev, "FUNNEL inport %d enabled\n", inport);
	return rc;
}
