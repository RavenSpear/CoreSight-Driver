...
static void tmc_etr_sg_table_dump(struct etr_sg_table *etr_table)
{
	...
	dev_info(sg_table->dev, "******* End of Table *****\n");
}
#else
					 ...
					 int node, void **pages)
{
	...
	dev_info(dev, "allocated buffer of size %ldKB in mode %d\n",
				  ...
				  int nr_pages, bool snapshot)
{
	...
	if (IS_ERR(etr_perf)) {
		dev_info(&csdev->dev, "Unable to allocate ETR buffer\n");
		return NULL;
	}
