...
void tmc_enable_hw(struct tmc_drvdata *drvdata)
{
	...
	u32 tmcctl = readl_relaxed(drvdata->base + TMC_CTL);
	pr_info("TMC Controller: %x\n", tmcctl);
}
...
static int tmc_read_prepare(struct tmc_drvdata *drvdata)
{
	...
	if (!ret)
		dev_info(&drvdata->csdev->dev, "TMC read start\n");
...
static int tmc_read_unprepare(struct tmc_drvdata *drvdata)
{
	...
	if (!ret)
		dev_info(&drvdata->csdev->dev, "TMC read end\n");
...
static int tmc_open(struct inode *inode, struct file *file)
{
	...
	dev_info(&drvdata->csdev->dev, "%s: successfully opened\n", __func__);
	return 0;
}
...
static int tmc_release(struct inode *inode, struct file *file)
{
	...
	dev_info(&drvdata->csdev->dev, "%s: released\n", __func__);
	return 0;
}
