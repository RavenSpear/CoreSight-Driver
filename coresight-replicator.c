				     ...
				     int inport, int outport)
{
	...
	id0 = readl_relaxed(drvdata->base + REPLICATOR_IDFILTER0);
	id1 = readl_relaxed(drvdata->base + REPLICATOR_IDFILTER1); 
	pr_info("Replicator ID FILTER 0: %x\n", id0);
	pr_info("Replicator ID FILTER 1: %x\n", id1);
			     ...
			     int outport)
{
	...
	if (first_enable)
		dev_info(&csdev->dev, "REPLICATOR enabled\n");
	return rc;
}
			       ...
			       int outport)
{
	...
	if (last_disable)
		dev_info(&csdev->dev, "REPLICATOR disabled\n");
}
